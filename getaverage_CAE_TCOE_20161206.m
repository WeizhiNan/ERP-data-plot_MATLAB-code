...
condition={'0Up','0Down','180Up','180Down'};
subj=[1:3 5:17 19:22];%被试序号
sT(1,1:2)=[396 561];%要处理的时间段N2
% sT(1,1:2)=[336 461];%要处理的时间段P3
% sT(1,1:2)=[371 471];%要处理的时间段N4
%%
...
% subno=1
% ei=1
        x=['a_' ,condition{1,ei} ,'_sub' ,int2str(subj(1,subno)), '.avg'];
...
disp('saving');
cd (savelocation);
xlswrite (['CAE_TCOE_',int2str(sT1), '_' ,int2str(sT2),'.xlsx'], da2,'mean');
clear all;
